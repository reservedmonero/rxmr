# The Current/Future Status of ZMQ in rXMR

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. rXMR
...
matching supported by both rXMR and ZMQ. The `format`, `context` and `event`
...
The rXMR daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The rXMR daemon also ensures that
...
The rXMR daemon will send a `txpool_add` pub exactly once for each
