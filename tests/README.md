...
Core tests take longer than any other rXMR tests, due to the high amount of computational work involved in validating core components.
...
To run only rXMR core tests (after building):
...
To run only rXMR crypto tests (after building):
...
```bash
rxmrd --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `rxmr-wallet-rpc --rpc-bind-port 18090 --daemon-address 127.0.0.1:18881 --disable-rpc-login`. Finally, start tests by invoking `./blockchain.py` or `./speed.py`.
...
To run only rXMR hash tests (after building):
...
To run only rXMR performance tests (after building):
...
To run only rXMR unit tests (after building):
