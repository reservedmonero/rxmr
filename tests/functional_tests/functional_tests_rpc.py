...
# a main offline rxmrd, does most of the tests
# a restricted RPC rxmrd setup with RPC payment
# two local online rxmrds connected to each other
N_MONERODS = 4

# 4 wallets connected to the main offline rxmrd
# 1 wallet connected to the first local online rxmrd
# 1 offline wallet
N_WALLETS = 6
...
rxmrd_base = [builddir + "/bin/rxmrd", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--no-igd", "--p2p-bind-port", "rxmrd_p2p_port", "--rpc-bind-port", "rxmrd_rpc_port", "--zmq-rpc-bind-port", "rxmrd_zmq_port", "--zmq-pub", "rxmrd_zmq_pub", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "rxmrd_data_dir", "--log-level", "1", "--rpc-max-connections-per-private-ip", "100", "--rpc-max-connections", "100"]
rxmrd_extra = [
  ["--offline"],
...
wallet_base = [builddir + "/bin/rxmr-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--disable-rpc-login", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1", "--allow-mismatched-daemon-version"]
wallet_extra = [
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
  ["--daemon-port", "18180"],
...
for i in range(N_MONERODS):
  command_lines.append([str(18180+i) if x == "rxmrd_rpc_port" else str(18280+i) if x == "rxmrd_p2p_port" else str(18380+i) if x == "rxmrd_zmq_port" else "tcp://127.0.0.1:" + str(18480+i) if x == "rxmrd_zmq_pub" else builddir + "/functional-tests-directory/rxmrd" + str(i) if x == "rxmrd_data_dir" else x for x in rxmrd_base])
  if i < len(rxmrd_extra):
    command_lines[-1] += rxmrd_extra[i]
  outputs.append(open(FUNCTIONAL_TESTS_DIRECTORY + '/rxmrd' + str(i) + '.log', 'a+'))
